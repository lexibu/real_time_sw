...
def plot_figures_dsco_07days(number_of_days=30):
    ...
    os.chdir("C:/Users/Lexi-Admin/Documents/real_time_sw")
    ...
    t_end = df_dscovr.index.min() + datetime.timedelta(minutes=1440 * 7)
    ...
    while t_end <= df_dscovr_hc.index[-1]:
        ...
        t1 = datetime.datetime.fromtimestamp(
            t_start.timestamp(), datetime.timezone.utc
        ).strftime("%Y-%m-%d_%H-%M-%S")
        t2 = datetime.datetime.fromtimestamp(
            t_end.timestamp(), datetime.timezone.utc
        ).strftime("%Y-%m-%d_%H-%M-%S")

        fig_name = (
            f"figures/historical/dscovr/07days/"
            f" sw_dscovr_parameters_07days_{t1}_{t2}.png"
        )

        # If the folder does not exist, create it
        Path(f"figures/historical/dscovr/07days/").mkdir(parents=True, exist_ok=True)
        ...
        _ = axs1.plot(
            df_dscovr.index, df_dscovr.bx_gsm, "r-", lw=lw, ms=ms, label=r"$B_x$"
        )
        ...
        _ = axs2.plot(
            df_dscovr.index, df_dscovr.by_gsm, "c-", lw=lw, ms=ms, label=r"$B_y$"
        )
        ...
        axs5.plot(
            df_dscovr_hc.index, df_dscovr_hc.np, color="fuchsia", lw=1, alpha=alpha
        )
        ...
        axs5a.plot(
            df_dscovr_hc.index, df_dscovr_hc.flux, color="oldlace", lw=1, alpha=alpha
        )
        ...
        plt.savefig(
            fig_name, bbox_inches="tight", pad_inches=0.05, format="png", dpi=200
        )
        plt.close("all")
